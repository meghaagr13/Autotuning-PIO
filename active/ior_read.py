...
data = pd.read_csv('../iorstats.txt', delim_whitespace=True)
...
print(X)
X = X.drop([0,1,3,4,6],1)
print("MMM")
print(X)
#FEATURES
#X['cb5']=X.iloc[:,1].astype(int)/X.iloc[:,0].astype(int)
X['cb3']=data['col13']
X['cb4']=data['col14']
X['cb5']=data['col15']
X['cb6']=data['col16']
...
#X['process']=data['col4']
print(X[0:5])
#print(X['process'])#print(X)
#NORMALIZING 
#print(data.head())
#print(data['col6'].head())
...
#train_X, test_X, train_Y, test_Y = train_test_split(X_total,Y,test_size=0.3,shuffle=True) 
y = Y
X = X_total
print(X[0:5])
...
xgb_model =xgb_model = xgb.XGBRegressor(objective="reg:linear", random_state=42, max_depth=10,n_estimators=1000)
...
modelfile = 'finaliorxgb_read.sav'
joblib.dump(xgb_model, open(modelfile, 'wb'))
scaler_filename = "scaleriorxgb_read.save"
