...
#    k.plot.bar(x='config',y=[2,17,5,20],title=f,figsize=(15,10),legend=True,fontsize=10,ax=axarr[i])
    k.plot.bar(x='config',y=[2,17,5,20],title=f,figsize=(15,10),legend=False,color=('sandybrown','dodgerblue','lightcoral','darkmagenta'),fontsize=10,ax=axarr[i])
    if i % 4 == 0:
        axarr[i].legend(["read_default","read_active","write_default","write_active"],ncol=2)
