...
for f in sorted(set(df['totalProcesses'])):
    ...
    k.plot.bar(x='config',y=[2,21,5,24],legend=False,figsize=(15,10),color=('sandybrown','dodgerblue','lightcoral','darkmagenta'),fontsize=10,ax=axarr[i])
    if(i == 2):
        axarr[i].legend(["read_default","read_active","write_default","write_active"],prop={'size': 8})
