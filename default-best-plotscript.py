...
for f in sorted(set(df['totalProcesses'])):
    ...
    k.plot.bar(x='config',y=[2,21,5,24],figsize=(15,10),legend=False,color=('sandybrown','dodgerblue','lightcoral','darkmagenta'),fontsize=10,ax=axarr[i])
    if(i == 2):
        axarr[i].legend(["read_default","read_active","write_default","write_active"],prop={'size': 10})
    axarr[i].tick_params(axis='x',labelrotation=0,labelsize=10)
    axarr[i].set_xlabel('')
    axarr[i].set_ylabel('I/O Bandwidth')
    i = i+1
